...
REF_AUDIO = os.environ.get("MIMO_REF_AUDIO", "")

def get_ref_audio_data():
    path = REF_AUDIO
...
def tts_from_file(input_path, output_path, voice_desc=""):
    ...
    with open(input_path, "r", encoding="utf-8") as f:
        text = f.read()
    ref_data = get_ref_audio_data()
    if not ref_data:
        print("need MIMO_REF_AUDIO pointing to a wav file", file=sys.stderr)
        sys.exit(1)
    payload = json.dumps({
        "model": MODEL,
        "messages": [
            {"role": "user", "content": voice_desc or ""},
            {"role": "assistant", "content": text}
        ],
        "audio": {"format": "wav", "voice": ref_data}
    }).encode()
